...
class SecurityManager:
    ...
    def validate_file_upload(self, uploaded_file) -> Dict[str, Any]:
        ...
        try:
            # Get max file size from production config if available
            try:
                from config.production import production_config
                max_size = production_config.max_file_size
            ...
            if file_size > max_size:
                ...
                validation_result['errors'].append(f"File size ({file_size / 1024 / 1024:.1f}MB) exceeds maximum allowed size ({max_size_mb:.0f}MB)")
...
# Use production config secret key if available, otherwise generate one
try:
    from config.production import production_config
    _secret_key = production_config.secret_key
except (ImportError, AttributeError):
    _secret_key = secrets.token_urlsafe(32)

security_manager = SecurityManager(_secret_key)
